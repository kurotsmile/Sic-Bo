class TaiXiu_MD5{
    ...
    show(){
        var d_cur=new Date();
        this.id_session="tx"+d_cur.getDay()+""+d_cur.getDate()+""+d_cur.getMonth()+d_cur.getFullYear();
        ...
        var transformValue = $('#tx').css('transform');
        taixiu.scaleX = 1; 
        if (transformValue !== 'none') {
            var scaleMatch = transformValue.match(/matrix\(([^,]+),/);
            if (scaleMatch) taixiu.scaleX = parseFloat(scaleMatch[1]);
        }
        ...
        if(taixiu.scaleX>=1){
            $("#tx").draggable({
                ...
                drag: function(event, ui) {
                    ui.position.left = ui.position.left / taixiu.scaleX;
                    ui.position.top = ui.position.top / taixiu.scaleX;
                },
    ...
    mo_dia_xong(){
        ...
        if(taixiu.money_bet!=0){
            if(w.user_login!=null){
                ...
                taixiu.obj_h_temp["username"]=w.user_login.username;
                taixiu.obj_h_temp["money_bet"]=taixiu.money_bet;
                taixiu.obj_h_temp["date"]=cr.getDateCur();

                if(taixiu.is_tai_bet==is_tai){
                    var money_reward=(taixiu.money_bet*2);
                    var new_money=parseInt(w.user_login.money)+money_reward;
                    taixiu.obj_h_temp["money_reward"]=money_reward;
                    taixiu.obj_h_temp["status"]="thang";

                    w.update_user_money(new_money.toString(),()=>{
                        taixiu.update_rank(taixiu.obj_h_temp.money_reward);
                        $.toast({
                            heading: 'Tài xỉu MD5',
                            text: 'Chúc mừng bạn đã thắng với số tiền +'+w.formatVND(money_reward)+" !",
                            icon: 'success',
                            loader: true,
                            loaderBg: '#9EC600'
                        });
    ...
    done_bet(){
        ...
        if(w.user_login==null){
            ...
            if(parseInt(taixiu.money_bet)<=parseInt(w.user_login.money)){
                var obj_user_bet={};
                var new_money=parseInt(w.user_login.money)-parseInt(taixiu.money_bet);
                obj_user_bet["username"]=w.user_login.username;
                obj_user_bet["sel"]=sel;
                obj_user_bet["money_bet"]=taixiu.money_bet;
                obj_user_bet["date"]=cr.getDateCur();
    
                w.update_user_money(new_money.toString(),()=>{
                    cr_realtime.add("tx",taixiu.id_session+"/"+taixiu.name_session+"/"+w.user_login.username,obj_user_bet);
                    $.toast({
                        heading: 'Tài Xỉu MD5',
    ...
    rank(){
        ...
        this.msg(html_rank,"Bảng xếp hạng",()=>{
            cr_realtime.list_one("tx/rank",datas=>{
                $("#all_item_rank").empty();
                datas.sort(function(a, b) { return parseInt(b.money) - parseInt(a.money);});
